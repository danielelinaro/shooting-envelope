...
class BDFIntegerSteps(OdeSolver):
    ...
    def _step_impl(self):
        ...
        print('_step_impl(%.3f)> h_abs = %f (%g*%f)' % (t, h_abs, self.h_abs/self.min_step, self.min_step))
        ...
        while not step_accepted:
            ...
            if not converged:
                ...
                if h_abs == h_abs_prev:
                    print('_step_impl(%.3f)> can not reduce the step any further.' % t)
                    ipdb.set_trace()
...
class BDFEnv(OdeSolver):
    def __init__(self, fun, t0, y0, t_bound, T_guess, max_step=np.inf,
                 ...
                 **extraneous):
        warn_extraneous(extraneous)
        super(BDFEnv, self).__init__(lambda t,y: self._envelope_fun(t,y,None),
