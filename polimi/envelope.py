...
class RungeKuttaEnvelope(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, T_guess, max_step=np.inf,
        ...
        super(RungeKuttaEnvelope, self).__init__(lambda t,y: self._envelope_fun(t,y,fun,
                                                                                T_guess=None,
                                                                                rtol=rtol,atol=atol),
                                                 t0, y0, t_bound, vectorized,
        ...
        #####
        self.f = self._envelope_fun(t0,y0,fun,T_guess,rtol=rtol,atol=atol)
    ...
    def _envelope_fun(self,t,y,fun,T_guess=None,method='RK45',**options):
        if T_guess is None:
            T_guess = self.T
...
def _one_period(t,y,fun,T_guess,method='RK45',full_output=True,**options):
    # find the equation of the plane containing y and
    # orthogonal to fun(t,y)
    f = fun(t,y)
    w = f/np.linalg.norm(f)
